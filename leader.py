...
class Node:
    def __init__(self, node_id, is_faulty=False):
        self.sender_sk = SigningKey.generate(curve=NIST384p)
    ...
    def send_block_to_node(self,header,id,node, port, block):
        ...
        header_dict = {"header":header,"id":id,"hash_block":hash_block,"pubkey":self.sender_vk.to_string().hex()}
    ...
    def handle_client_connection(self,client_socket, address):
        ...
        try:
            ...
            return
        print(f"Received message from {address}: {header}")
        if header.startswith("REQUEST"):
            if not check_sign_header(data_dict["header_dict"],data_dict["signed_header"]):
                ...
                new_transaction = create_transaction_from_data(data_dict["block_dict"])
                if not self.blockchain.add_transaction(new_transaction) :
                    print(f"error in transcation")
                elif len(self.blockchain.pending_transactions) == 5 :
                    ...
                    print(f"Listening on port {7778}...")
                    ...
                    with self.lock:
                        self.commit_count = 1
                    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    server_socket.bind(('0.0.0.0', 7779))
                    server_socket.listen(5)
                    print(f"Listening on port {7779}...")
    ...
    try:
        while True:
            ...
            client_handler = threading.Thread(
                target=node.handle_client_connection,
